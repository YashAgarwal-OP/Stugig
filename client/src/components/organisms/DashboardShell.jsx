    ...
    <div className="flex min-h-screen bg-[#f3f4f5]">
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-60 bg-white border-r border-[#e7e8e9] flex flex-col z-30 shadow-sm">
        ...
        <div className="border-t border-[#e7e8e9] p-4 flex items-center gap-3">
          <button onClick={() => navigate('/profile/edit')} className="flex items-center gap-3">
            ...
            <Avatar src={user?.profilePhotoUrl} name={user?.name} size="sm" className="hover:opacity-80 transition-opacity" />
          </button>
          <button onClick={handleLogout} className="ml-auto text-[#777587] hover:text-[#ba1a1a] transition-colors" aria-label="Log out">
